...
const ChatLabel = ({ openMenu, setOpenMenu }) => {
  ...
  const renameHandler = async () => {
    try {
      ...
      if (data.success) {
        fetchUserChats(), setOpenMenu({ id: 0, open: false });
  ...
  return (
    <div
      onClick={selectChat}
      className="flex items-center relative justify-between p-2 text-white/80 hover:bg-white/10 rounded-lg text-sm group cursor-pointer">
